 ...
 * Supports both One-Time & Installment Plans (Kisht)
 * Rule: If remainingFee > 0 and past dueDate, add ₹150 for every 2 days overdue.
 */
export function calculateStudentFee(student) {
  ...
  if (remainingFee <= 0 && (!installments.length || installments.every(i => i.status === 'Paid'))) {
    return {
      course,
      feeType,
      remainingFee: 0,
