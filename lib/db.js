...
export const db = {
  ...
  updateStudent(id, updates) {
    ...
    const index = data.students.findIndex(s =>
      ...
      s.phone === cleanId ||
      String(s._id || '').toLowerCase() === cleanId
    );
    if (index === -1) throw new Error("Student not found");
  ...
  deleteAttendanceRecord(id) {
    ...
    if (data.attendance.length !== initialLen) {
      writeDb(data);
      return true;
    }
    return false;
  },
  deleteAttendanceLogs({ startDate, endDate, date, studentId }) {
    const data = readDb();
    const initialLen = (data.attendance || []).length;
    data.attendance = (data.attendance || []).filter(a => {
      if (date && a.date === date) return false;
      if (startDate && endDate && a.date >= startDate && a.date <= endDate) {
        if (!studentId || a.studentId === studentId || a.rollNo === studentId) return false;
      }
      if (studentId && (a.studentId === studentId || a.rollNo === studentId)) return false;
      return true;
    });
  ...
  approveStudent(id, { rollNo, course, feeType = 'single', remainingFee, dueDate, installments = [], totalInstallments = 1 }) {
    ...
    const rollExists = data.students.some((s, idx) => idx !== index && s.rollNo && s.rollNo.toUpperCase() === cleanRollNo);
    if (rollExists) {
