...
export const db = {
  ...
  recordPunchIn({ studentId, date, time, lat, lng, distance, selfieImg, status = "Present", remarks = "" }) {
    const data = readDb();
  ...
  recordPunchOut({ studentId, date, time, lat, lng, distance, selfieImg, studySummary = "", remarks = "" }) {
    const data = readDb();
    const record = data.attendance.find(a => a.studentId === studentId && a.date === date);
    if (!record || !record.punchInTime) {
      throw new Error("Cannot punch out without punching in first today.");
    }
